...
exports.getLogin = (req, res, next) => {
  ...
  if (messageLogin && messageLogin.length > 0) {
    ...
    messageAdminProtectionRoute &&
    messageAdminProtectionRoute.length > 0
  ) {
    message[0] = "Failed";
    message[1] = messageAdminProtectionRoute[0];
  } else if (
    messageUserProtectionRoute &&
    messageUserProtectionRoute.length > 0
  ) {
    message[0] = "Failed";
...
exports.postRegister = (req, res, next) => {
  ...
  bcrypt.hash(password, 12).then((hashedpassword) => {
    ...
    const redirectUrl =
      process.env.BASE_URL_EMAIL +
        `/auth/email-confirmation?email=${email}&activationToken=${key}` ||
      `http://localhost:3000/auth/email-confirmation?email=${email}&activationToken=${key}`;
    sendMail.sendMail(email, redirectUrl);

    newUser.save();
    req.flash(
      "regist-success",
      "Silahkan aktivasi akun anda melalui email yang kami kirimkan"
    );
...
exports.postEmailConfirmation = (req, res, next) => {
  ...
  User.findOne({ email: email }).then((user) => {
    ...
    req.flash(
      "regist-success",
      "Aktivasi gagal, pastikan email anda sudah benar"
    );
    return res.redirect("/auth/login");
  });
};
